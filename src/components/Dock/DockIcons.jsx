...
import {
  selectOpenApps,
  selectOpenFolders
} from "../../redux/windows/windows.selectors";
...
const DockIcons = ({
  ...
  function onAppClick() {
    appIsRunning(this.windowOpts.id)
      ? onMaximize(this.windowOpts.id)
      : openWindowAndSetFocused(this.windowOpts);
  }
  ...
  function onMaximize(id) {
    const { elementId, width, height, x, y } = find(
      openApps.concat(openFolders),
      { id: id }
    );
    ...
    <React.Fragment>
      {icons.map((icon, index) => (
        <div
          ...
          className={C(
            cls.dockIcon,
            appIsRunning(get(icon, ["windowOpts", "id"])) && cls.appRunning
          )}
        >
          <div className={cls.title}>
            {get(icon, ["windowOpts", "title"], icon.title)}
          </div>
          <Icon {...icon} />
        </div>
      ))}
