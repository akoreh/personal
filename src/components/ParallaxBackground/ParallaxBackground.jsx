...
const ParallaxBackground = ({ layers }) => {
  const parallaxContainerRef = React.useRef();
  const parallaxRef = React.useRef();

  const scrollToBottom = () =>
    parallaxRef.current.scrollTo(0, parallaxRef.current.clientHeight);
  ...
  const scrollToTop = () => {
    ...
    if (IS_DEV) {
      ...
      TweenMax.to(target, SCROLL_TO_TOP_DURATION, {
        scrollTo: { x: 0, y: 0 },
        delay
      });
    }
  ...
  return (
    <div className={cls.parallaxBackground} ref={parallaxContainerRef}>
      <div className={cls.parallax} ref={parallaxRef}>
        {layers.map(layer => (
          <ParallaxLayer key={layer.src} {...layer} />
        ))}
